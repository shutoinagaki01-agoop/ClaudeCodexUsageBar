...
片方だけを利用する場合は、メニューの **詳細設定 > 利用するサービス > Claude / Codex** で、使わないサービスのチェックを外してください。オフのサービスは残量・エラーを表示せず、自動更新・手動更新の対象から外れます。設定は再起動後も保持されます（既定は両方オン）。

再びオンにすると、そのサービスの残量をすぐに取得します。両方オフの場合もメニューバーに使用量アイコン（文字表示の場合は「使用量」）を残し、メニューから再度オンにできます。

オフにする前に開始した通信や公式 CLI の認証更新は完了する場合がありますが、その取得結果は表示や通知に反映しません。
...
## 事前準備
アプリのインストール前後は問いません。Claude / Codex CLI が保存する OAuth 認証情報を利用するため、利用するサービスの CLI のみ準備してください。
...
| 項目 | 動作 |
|---|---|
| 詳細設定 > 利用するサービス > Claude / Codex | サービスごとに利用をオン／オフ。オフにすると表示・取得を停止し、オンにすると即座に取得する |
| Claude/Codexの残量を手動で更新 | オンのサービスだけを再取得 (`⌘R`)。項目名も対象に合わせて変わる。Claude 認証切れ時は公式 CLI の PTY 更新も実行する |
